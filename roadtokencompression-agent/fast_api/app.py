...
# 业务运行时：(config, road_runtime)，由 lifespan 初始化。
_runtime = None
...
async def lifespan(_: FastAPI):
    ...
    try:
        ...
        # 模型加载：路端 Qwen 视觉编码 + Token 筛选 + INT8 量化运行时。
        config = load_config(RUNTIME_CONFIG_PATH, role="road")
        road_runtime = build_road_runtime(config)
        _runtime = (config, road_runtime)
        logger.info(
            "Road token compression runtime loaded (mock=%s)",
            config["runtime"]["mock"],
        )
    except Exception as exc:
...
async def agent_function(
    ...
    # TODO: 在此处编写你的业务逻辑
    # ─── 开发者替换区: 在此处添加你的业务逻辑 ───
    # 路端：从 NATS 获取车端采样帧，执行 Qwen 视觉编码 + Token 筛选 + INT8。
    stage_started = time.monotonic()
    frame_dir = None
    try:
        config, road_runtime = _runtime
        payload_in = decode_data.get("processed_data", {})
        if not isinstance(payload_in, dict):
            payload_in = decode_data
        sample_id = payload_in.get("sample_id", decode_data.get("sample_id", ""))
        task_type = str(
            parameters.get("task_type") or os.getenv("TASK_TYPE", "ours")
        ).strip().lower()
        if task_type == "baseline":
            # 传统链路：边端不压缩，原样转发完整视频给云端
            video_base64 = payload_in.get("video_base64") or decode_data.get("video_base64")
            if not video_base64:
                raise ValueError("road_forward 需要上游 video_base64")
            processed = {
                "sample_id": sample_id,
            ...
            observe_performance_metrics(
                {"forwarded_video_bytes": float(len(video_base64) * 3 / 4)}
            )
        else:
            # 被测链路：Qwen 视觉编码 + Token 筛选 + INT8
            frames = payload_in.get("frames_base64") or decode_data.get("frames_base64") or []
            clip_paths = payload_in.get("clip_paths") or decode_data.get("clip_paths") or []
            if isinstance(clip_paths, str):
                clip_paths = [clip_paths]
            if not frames and not clip_paths:
                raise ValueError("上游数据缺少 frames_base64 或 clip_paths")

            work_dir = Path(config["paths"]["work_dir"])
            frame_dir = work_dir / "road_input"
            frame_dir.mkdir(parents=True, exist_ok=True)
            paths = [Path(str(value)) for value in clip_paths]
            for index, item in enumerate(frames):
                if isinstance(item, str):
                    data = item
                    filename = f"frame_{index:03d}.jpg"
                else:
                    data = item.get("data_base64") or item.get("data")
                    filename = item.get("filename") or f"frame_{index:03d}.jpg"
                if not data:
                    raise ValueError("帧数据缺少 data_base64")
                (frame_dir / filename).write_bytes(base64.b64decode(data))
                paths.append(frame_dir / filename)

            token_payload, timings, token_metrics = road_runtime.encode_clips(paths)
            token_wire_bytes = len(
                json.dumps(
                    token_payload,
                    ensure_ascii=False,
                    separators=(",", ":"),
            ...
            processed = {
                "sample_id": sample_id,
                "token_payload": json.dumps(token_payload, ensure_ascii=False),
            ...
            observe_performance_metrics(
                {
                    "token_keep_ratio": float(
                        token_metrics.get("token_keep_ratio_actual", 0.0)
                    ),
                    "retained_tokens": float(
                        token_metrics.get("retained_visual_tokens", 0.0)
                    ),
                    "token_bytes": float(token_wire_bytes),
                }
            )
    finally:
...
def _build_agent_card() -> AgentCard:
    ...
    skill = AgentSkill(
        id="road_token_compression",
        name="Road Token Compression Agent",
        description=(
            "Encodes vehicle frames with Qwen visual encoder, filters ~90% "
            "visual tokens, applies INT8 quantization, and publishes the "
            "token payload to the cloud agent through NATS."
        ),
        input_modes=["application/json", "text/plain"],
        output_modes=["text/plain"],
        tags=["road-agent", "token-compression"],
        examples=[
            "Compress visual tokens from upstream frames",
            (
                '{"task_description": "Compress visual tokens", '
                '"parameters": {"source_cluster": "vrc", '
                '"target_cluster": "vrc", '
                '"target_agent_id": "cloud-agent", '
    ...
    return AgentCard(
        name="Road Token Compression Agent",
        description=(
            "路端 Agent：Qwen 视觉编码，Token 目标保留 90%，INT8 量化，"
            "通过 NATS 将 Token 载荷发送给云端。"
        ),
        version="1.0.0",
...
app = FastAPI(title="Road Token Compression Agent", lifespan=lifespan)
