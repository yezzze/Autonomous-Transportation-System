#!/usr/bin/env bash
# 加载 scripts/local/cluster.env 并导出集群 A/B 与当前边缘集群变量。
...
LOCAL_CLUSTER="${LOCAL_CLUSTER:?请在 cluster.env 中设置 LOCAL_CLUSTER=a 或 b}"
...
case "${LOCAL_CLUSTER}" in
  a)
    ...
    PEER_HOST="${CLUSTER_B_HOST}"
  ;;
  b)
    EDGE_CLUSTER_ID="${EDGE_CLUSTER_ID:-${CLUSTER_B_EDGE_ID}}"
    LOCAL_HOST="${CLUSTER_B_HOST}"
    PEER_HOST="${CLUSTER_A_HOST}"
  ;;
  *)
    echo "LOCAL_CLUSTER 必须是 a 或 b，当前: ${LOCAL_CLUSTER}" >&2
    return 1 2>/dev/null || exit 1
  ;;
esac
...
export LOCAL_CLUSTER EDGE_CLUSTER_ID LOCAL_HOST PEER_HOST
export CLUSTER_A_HOST CLUSTER_B_HOST CLUSTER_A_EDGE_ID CLUSTER_B_EDGE_ID
