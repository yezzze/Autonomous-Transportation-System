...
from utils.transformation_utils import project_points_by_matrix_numpy, x1_to_x2
from opencood.utils.transformation_utils import gps_to_utm_transformation
...
def lidar_pose_to_box_corner(other_lidar_pose, ego_lidar_pose, gps=False):
    if gps:
        other_to_ego_matrix = gps_to_utm_transformation(other_lidar_pose, ego_lidar_pose)
    else:
    ...
    other_box_corner_in_ego = project_points_by_matrix_numpy(other_box_corner, other_to_ego_matrix)
    other_box_corner_in_ego = other_box_corner_in_ego.reshape((1, 8, 3))
    return other_box_corner_in_ego

def lidar_pose_to_oabb(other_lidar_pose, ego_lidar_pose, left_hand_coordinate=True, gps=False):
    other_box_corner_in_ego = lidar_pose_to_box_corner(other_lidar_pose, ego_lidar_pose, gps)
    other_oabb = bbx2oabb(other_box_corner_in_ego, left_hand_coordinate=left_hand_coordinate)[0]
