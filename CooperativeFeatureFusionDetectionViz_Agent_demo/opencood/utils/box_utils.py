...
def remove_large_pred_bbx(bbx_3d):
    ...
    bbx_z_min = torch.min(bbx_3d[:, :, 2], dim=1)[0]
    z_len = bbx_z_max - bbx_z_min
    # mask：哪些框的 z_len 大于 2
    large_mask = z_len > 2

    # 两种条件
    cond_large = torch.logical_and(x_len <= 12, y_len <= 6)
    cond_small = torch.logical_and(x_len <= 6, y_len <= 6)

    # 对每个样本单独判断用哪个条件
    index = torch.where(large_mask, cond_large, cond_small)
