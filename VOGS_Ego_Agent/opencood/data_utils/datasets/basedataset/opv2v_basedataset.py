...
class OPV2VBaseDataset(Dataset):
    def __init__(self, params, visualize, train=True):
        ...
        #co3
        self.load_occ_co3sop_256 = 'occ_co3sop_256' in params['input_source']
        self.load_occ_co3sop_256_single = 'occ_co3sop_256_single' in params['input_source'] # for single agent training 

        self.label_type = params['label_type'] # 'lidar' or 'camera'
        #so3
        if self.label_type == "occ":
            self.generate_object_center = self.generate_object_center_occ
        elif self.label_type == "camera":
            self.generate_object_center = self.generate_object_center_camera
        elif self.label_type == "lidar":
        ...
        for (i, scenario_folder) in enumerate(self.scenario_folders):
            ...
            for (j, cav_id) in enumerate(cav_list):
                ...
                for timestamp in timestamps:
                    ...
                    #co3
                    occ_co3sop_256 = os.path.join(cav_path, f'{timestamp}_voxels.npz')
                    occ_co3sop_256 = occ_co3sop_256.replace("OPV2V", "OPV2V/additional_50m") # for co3sop, we directly provide the voxel label without needing to process raw point cloud data, and the perception range is 50m on xy axis, which is larger than the original 40m in OPV2V.
                    # Keep a single-agent alias for the same co3sop label path to match
                    # configs that request `occ_co3sop_256_single`.
                    occ_co3sop_256_single = occ_co3sop_256
                    ...
                    self.scenario_database[i][cav_id][timestamp].update({
                        ...
                        #co3
                        'occ_co3sop_256': occ_co3sop_256,
                        'occ_co3sop_256_single': occ_co3sop_256_single,
    ...
    def retrieve_base_data(self, idx):
        ...
        for cav_id, cav_content in scenario_database.items():
            ...
            #co3
            if self.load_occ_co3sop_256:
                occ_co3sop_256_path = cav_content[timestamp_key]['occ_co3sop_256']
                data[cav_id]['occ_co3sop_256'] = occ_co3sop_256_path
            if self.load_occ_co3sop_256_single:
                occ_co3sop_256_single_path = cav_content[timestamp_key]['occ_co3sop_256_single']
                data[cav_id]['occ_co3sop_256_single'] = occ_co3sop_256_single_path
    ...
    #co3
    def generate_object_center_occ(self, *args, **kwargs):
        return np.zeros((100, 7)), np.zeros(100), [] # placeholder from VOGS-CP, which doesn't use camera bbox labels
