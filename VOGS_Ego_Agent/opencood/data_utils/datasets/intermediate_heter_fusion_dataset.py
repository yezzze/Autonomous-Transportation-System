...
def getIntermediateheterFusionDataset(cls):
    ...
    class IntermediateheterFusionDataset(cls):
        ...
        def get_item_single_car(self, selected_cav_base, ego_cav_base):
            ...
            #co3
            voxel_label_co3sop_256_single = selected_cav_base.get('occ_co3sop_256_single', None) # for single agent training

            semantic_pcd = selected_cav_base.get('occ_raw', None)
            semantic_xyz = None
            semantic_class = None
            label_mapping_co3sop = None
            if semantic_pcd:
                ...
                label_mapping_co3sop = np.zeros(24, dtype=np.int32) # used for direct label mapping from co3sop labels to labels used in VOGS, 24 is the number of classes in co3sop, the one extra label is the empty class, which is not contained in semantic point clouds of Semantic-OPV2V
                label_mapping_co3sop[[0, 1, 2, 5, 6, 7, 8, 9, 10, 11, 12, 14, 15, 17, 18, 20, 22]] = [ # 0 is unlabeled, 13 is empty (not specified here but in config, not used for supervision, only for masking out empty voxels when calculating loss)
                    13, 1, 2, 4, 5, 5, 6, 7, 8, 9, 11, 3, 12, 10, 4, 8, 3
                ]
                # 0: unlabled, 1: building, 2:fence, 3: Terrain, 4: Pole, 5: Road, 6: Sidewalk, 7: Vegetation, 8: Vehicle, 9: Wall, 10: Guardrail, 11: traffic sign, 12: bridge, 13: empty
            ...
            label_dict = self.post_processor.generate_label(
                ...
                #co3
                voxel_label_co3sop=voxel_label_co3sop_256_single,
                label_mapping=label_mapping_co3sop
            )
            ...
            selected_cav_processed.update(
                {
                    ...
                    "semantic_class": semantic_class,
                    #co3
                    "label_mapping_co3sop": label_mapping_co3sop
                }
            )
        ...
        def __getitem__(self, idx):
            ...
            for _i, cav_id in enumerate(cav_id_list):
                ...
                if not self.visualize:
                    ...
                    elif "camera" in sensor_type and self.label_type == "camera":
                        self.generate_object_center = eval(f"self.generate_object_center_camera")
                    elif self.label_type == "occ":
                        self.generate_object_center = eval(f"self.generate_object_center_occ")
                # need discussion. In test phase, use lidar label.
                # 07/16/2025 For vision only, we use camera label
                else:
                    if self.label_type == "occ":
                        self.generate_object_center = eval(f"self.generate_object_center_occ")
                    else:
            ...
            #co3
            if selected_cav_processed["label_mapping_co3sop"] is not None:
                    label_mapping_co3sop = selected_cav_processed["label_mapping_co3sop"]
            else:
                label_mapping_co3sop = None
            # ===
            for modality_name in self.modality_name_list:
            ...
            #co3
            voxel_label_co3sop_256 = ego_cav_base.get('occ_co3sop_256', None)
            ...
            label_dict = self.post_processor.generate_label(
                ...
                #co3
                voxel_label_co3sop=voxel_label_co3sop_256,
                label_mapping = label_mapping_co3sop
            )
