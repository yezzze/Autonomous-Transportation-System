# 智能体编排层接口流程 v2

> 本版本在原“智能体编排层接口流程 v1”基础上新增 **MC：记忆中心（Memory Center）**。
> MC 不是普通业务智能体，而是编排层控制面组件，负责记忆抽取、记忆包生成、沙箱挂载、远端委派记忆、执行后记忆回收与本地写回。

---

## 0. 核心设计原则

### 0.1 每个设备都是独立本体

系统中的每个设备都可视为一个独立客户端：

```text
设备 A = 一个独立客户端 / 本体 A
设备 B = 一个独立客户端 / 本体 B
```

每个设备拥有：

```text
1. 本地应用管理层 APP
2. 本地智能体编排层 Orchestration
3. 本地智能体运行层 Runtime
4. 本地记忆中心 MC
5. 本地记忆库 Memory Store
```

设备之间可以互相调用能力，但默认不共享本体私有记忆。

---

### 0.2 Agent 沙箱只能访问专属记忆目录

Agent 由 K8s 沙箱调度，不同 Agent 实例、不同 Workflow、不同设备之间不共享文件系统。

每个 Agent 实例启动前，MC 会为其生成专属记忆包，并由 ASD / ALCM 挂载到沙箱内：

```text
/sandbox/memory
```

Agent 沙箱只能访问该目录，不能访问：

```text
1. 其他 Agent 的记忆目录
2. 其他 Workflow 的记忆目录
3. 本设备全量长期记忆库
4. 其他设备的本体记忆
```

---

### 0.3 远端调用采用双归属记忆

当设备 A 调用设备 B 的 Agent 时，执行产生的记忆具有双重归属：

```text
1. 调用方记忆：
   设备 A 记录“我调用了设备 B 的某个 Agent，结果如何”。

2. 被调用方本地 Agent 记忆：
   设备 B 记录“我的某个 Agent 被设备 A 的某个 Workflow 调用，执行了什么任务，效果如何”。
```

被调用设备可以保存：

```text
1. 调用事实
2. 调用方设备 ID
3. 调用方 Workflow ID
4. 任务类型
5. 执行结果摘要
6. 性能指标
7. 本地 Agent 经验
```

但不得保存调用方未授权暴露的私有原始记忆、敏感上下文或全量用户记忆。

---

## 1. 智能体发现

```mermaid
sequenceDiagram
autonumber

box 终端
    participant D_ARDC as 智能体注册与发现中心<br>(ARDC)
end

box 边缘
    participant E_ARDC as 智能体注册与发现中心<br>(ARDC)
end

note over D_ARDC,E_ARDC: 智能体发现过程
loop
    D_ARDC->>E_ARDC: 广播开放的智能体列表(接口、功能描述)
end

loop
    E_ARDC->>D_ARDC: 广播开放的智能体列表(接口、功能描述)
end


```

说明：

* ARDC 仍然只负责智能体发现。
* MC 不参与 Agent 能力注册，但 ARDC 中可记录 Agent 的记忆策略摘要。
* 设备之间不广播本体私有记忆。
* 可广播的信息包括：

  * Agent 是否允许远端调用；
  * Agent 是否允许保存远端调用经验；
  * Agent 是否允许读取本地 Agent Memory；
  * Agent 是否允许接收委派记忆包。

---

## 2. 单主体工作流编排

```mermaid
sequenceDiagram
autonumber

participant APP as 1.应用管理层<br>(APP)

box 设备本体 / 智能体编排层
    participant AOE as 2.1 智能体编排引擎<br>(AOE)
    participant AWM as 2.2 智能体工作流管理<br>(AWM)
    participant ASD as 2.3 智能体调度与部署<br>(ASD)
    participant ARDC as 2.4 智能体注册与发现中心<br>(ARDC)
    participant RRDC as 2.5 资源注册与发现中心<br>(RRDC)
    participant MC as 2.6 记忆中心<br>(MC)
end

box 运行层 / K8s 沙箱
    participant ALCM as 生命周期管理<br>(ALCM)
    participant POD as Agent 沙箱<br>(SBOX / POD)
end

participant STORE as 本体记忆库<br>(Memory Store)
participant RES as 资源抽象层<br>(Resource)

APP->>AOE: 开始编排<br>(指导文件, user_input, app_id)

AOE->>MC: 创建 memory_scope<br>(device_id, user_id, app_id, workflow_id)
MC->>STORE: 检索本体长期记忆
STORE->>MC: 返回候选记忆

AOE->>ARDC: 请求智能体列表
ARDC->>AOE: 返回智能体列表<br>(镜像 + 实例 + 记忆策略摘要)

AOE->>AOE: 基于指导文件、用户输入、Agent 列表、记忆摘要<br>生成任务图 execution_plan

loop 为每个任务 / Agent 准备记忆包
    AOE->>MC: create_memory_bundle<br>(task_id, agent_id, memory_policy)
    MC->>STORE: 检索相关本体记忆与 Agent 记忆
    STORE->>MC: 返回候选记忆
    MC->>MC: 过滤、摘要、脱敏、排序
    MC->>MC: 生成 MemoryBundle
    MC->>MC: 物化专属记忆目录<br>/memory-bundles/{workflow_id}/{agent_instance_id}/
    MC->>AOE: 返回 bundle_id 与 memory_mount_spec
end

AOE->>AWM: 生成并启动工作流<br>(execution_plan, memory_bundle_refs)
activate AWM

AWM->>ASD: 部署智能体<br>(镜像ID, 资源配置, memory_mount_spec)
ASD->>RRDC: 请求资源列表
RRDC->>ASD: 返回资源列表

ASD->>ALCM: 部署智能体<br>(image_id, resource_config, memory_mount_spec)
ALCM->>RES: 请求资源分配
RES->>ALCM: 资源分配成功

ALCM->>POD: 创建 Agent 沙箱并挂载专属记忆目录<br>/sandbox/memory
POD->>ALCM: 创建成功

ALCM->>ASD: 智能体部署成功<br>(智能体接口)
ASD->>ARDC: 注册智能体实例
ARDC->>ASD: 注册成功
ASD->>AWM: 部署成功

AWM->>POD: 调用 Agent<br>(task_input)
POD->>POD: 读取 /sandbox/memory/input
POD->>POD: 写入 /sandbox/memory/output
POD->>AWM: 返回执行结果

AWM->>MC: 回收 Agent 输出<br>collect_outbox(bundle_id)
MC->>MC: 写入候选记忆<br>(task_result, agent_experience)
MC->>AWM: 写入成功

AWM->>MC: 工作流完成后提交 workflow_summary
MC->>STORE: 写回本体长期记忆
STORE->>MC: 写入成功

AWM->>AOE: 工作流完成
AOE->>APP: 返回编排结果

deactivate AWM
```

说明：

* Agent 不直接访问 MC 全量记忆库。
* Agent 只访问 `/sandbox/memory`。
* MC 在 Agent 启动前为其生成专属 MemoryBundle。
* Agent 执行后将候选记忆写入 `/sandbox/memory/output`。
* MC 回收 output 后决定是否写入长期记忆。

---

## 3. 跨主体工作流编排

```mermaid
sequenceDiagram
autonumber

box 设备 A / 调用方本体
    participant A_APP as 1.应用管理层<br>(APP_A)
    participant A_AOE as 2.1 编排引擎<br>(AOE_A)
    participant A_AWM as 2.2 工作流管理<br>(AWM_A)
    participant A_ARDC as 2.4 注册发现<br>(ARDC_A)
    participant A_MC as 2.6 记忆中心<br>(MC_A)
    participant A_STORE as 本体 A 记忆库
end

box 设备 B / 被调用方本体
    participant B_AOE as 2.1 编排引擎<br>(AOE_B)
    participant B_AWM as 2.2 工作流管理<br>(AWM_B)
    participant B_ARDC as 2.4 注册发现<br>(ARDC_B)
    participant B_ASD as 2.3 调度部署<br>(ASD_B)
    participant B_MC as 2.6 记忆中心<br>(MC_B)
    participant B_STORE as 本体 B 记忆库
    participant B_POD as Agent 沙箱<br>(POD_B)
end

A_APP->>A_AOE: 开始编排<br>(指导文件, user_input)

A_AOE->>A_MC: 创建 memory_scope_A
A_MC->>A_STORE: 检索本体 A 相关记忆
A_STORE->>A_MC: 返回候选记忆

A_AOE->>A_ARDC: 请求智能体列表
A_ARDC->>A_AOE: 返回本地 + 远端 Agent 列表

A_AOE->>A_AOE: 拆解任务图
A_AOE->>A_AOE: 判断部分子任务需要调用设备 B 能力

A_AOE->>A_MC: 创建 delegated_memory_bundle<br>(remote_task, target_device_id, target_agent_id)
A_MC->>A_STORE: 检索允许暴露的相关记忆
A_STORE->>A_MC: 返回候选记忆
A_MC->>A_MC: 最小化、脱敏、权限过滤
A_MC->>A_AOE: 返回 delegated_bundle

A_AOE->>B_AOE: 分发子任务图<br>(subtask_graph, delegated_bundle, memory_policy, session_timeout)

B_AOE->>B_ARDC: 请求本地 Agent 列表
B_ARDC->>B_AOE: 返回本地 Agent 列表

B_AOE->>B_MC: 接收远端调用请求<br>(caller_device_id, caller_workflow_id, delegated_bundle)
B_MC->>B_STORE: 检索目标 Agent 本地经验记忆
B_STORE->>B_MC: 返回 agent_memory_excerpt

B_MC->>B_MC: 合并 delegated_bundle 与本地 Agent 经验
B_MC->>B_MC: 生成远端 Agent 沙箱专属记忆目录

B_AOE->>B_AWM: 生成并启动远端工作流<br>(memory_bundle_refs)
activate B_AWM

B_AWM->>B_ASD: 部署远端 Agent<br>(memory_mount_spec)
B_ASD->>B_POD: 创建沙箱并挂载 /sandbox/memory

B_POD->>B_POD: 读取 input/delegated_context
B_POD->>B_POD: 读取 local/agent_memory_excerpt
B_POD->>B_POD: 执行任务
B_POD->>B_POD: 写入 output/result
B_POD->>B_POD: 写入 output/writeback_to_caller
B_POD->>B_POD: 写入 output/writeback_to_local

B_POD->>B_AWM: 返回任务结果
B_AWM->>B_MC: collect_outbox(bundle_id)

B_MC->>B_STORE: 保存 writeback_to_local<br>(agent_remote_experience, delegation_memory)
B_STORE->>B_MC: 保存成功

B_MC->>B_AWM: 返回 result + writeback_to_caller
B_AWM->>B_AOE: 远端工作流完成
deactivate B_AWM

B_AOE->>A_AOE: 返回远端结果<br>(result, writeback_to_caller)

A_AOE->>A_MC: 保存远端调用结果<br>(remote_call_result, workflow_memory)
A_MC->>A_STORE: 写入本体 A 记忆库
A_STORE->>A_MC: 写入成功

A_AOE->>A_AWM: 生成并启动本地工作流<br>(含远程结果)
A_AWM->>A_APP: 返回最终结果
```

说明：

* 调用方 A 只向 B 发送经过脱敏和最小化的 delegated_memory_bundle。
* 被调用方 B 可以结合自己的本地 Agent Memory 执行任务。
* B 的 Agent 执行后会产生两类写回：

  * `writeback_to_caller`：返回给 A_MC。
  * `writeback_to_local`：保存到 B_MC 的 Agent Memory。
* B 不保存 A 未授权暴露的私有原始上下文。

---

## 4. 跨主体工作流重编排

```mermaid
sequenceDiagram
autonumber

box 设备 A / 调用方
    participant A_AOE as 编排引擎<br>(AOE_A)
    participant A_AWM as 工作流管理<br>(AWM_A)
    participant A_ARDC as 注册发现<br>(ARDC_A)
    participant A_MC as 记忆中心<br>(MC_A)
end

box 新设备 C / 新能力提供方
    participant C_AOE as 编排引擎<br>(AOE_C)
    participant C_MC as 记忆中心<br>(MC_C)
end

box 旧设备 B / 旧能力提供方
    participant B_AOE as 编排引擎<br>(AOE_B)
    participant B_AWM as 工作流管理<br>(AWM_B)
    participant B_MC as 记忆中心<br>(MC_B)
end

note over A_AWM: 工作流执行过程中

activate A_AWM
activate B_AWM

A_AWM->>B_AWM: 调用远端工作流
B_AWM--xA_AWM: 超时 / 错误 / Agent 失效

A_AWM->>A_MC: 写入远端调用失败候选记忆<br>(failed_remote, task_id, reason)
A_MC->>A_MC: 暂存 failure_case

A_AWM->>A_AOE: 请求重编排<br>(failed_task, failed_remote_url)

A_AOE->>A_MC: 查询历史失败经验<br>(failure_case, remote_call_result)
A_MC->>A_AOE: 返回失败节点、替代节点、历史策略

A_AOE->>A_ARDC: 请求运行时智能体列表
A_ARDC->>A_AOE: 返回终端 + 边缘 + 远端 Agent 列表

A_AOE->>A_AOE: 基于 ARDC + MC 记忆重新筛选 Agent
A_AOE->>A_AOE: 拆解新任务图

A_AOE->>A_MC: 创建新的 delegated_memory_bundle<br>(排除 failed_remote)
A_MC->>A_AOE: 返回新委派记忆包

A_AOE->>C_AOE: 分发子任务图<br>(new_subtask, delegated_bundle)
C_AOE->>C_MC: 接收委派记忆包并创建沙箱记忆目录
C_MC->>C_AOE: 创建成功
C_AOE->>A_AOE: 子任务图工作流启动成功

A_AOE->>A_MC: 记录重编排决策<br>(old_remote, new_remote, strategy)
A_MC->>A_AOE: 写入成功

A_AOE->>A_AWM: 停止旧远端工作流引用
deactivate A_AWM

B_AWM->>B_MC: 记录本地远端调用失败经验<br>(caller_device, workflow_id, failure_reason)
B_MC->>B_MC: 写入本地 Agent Memory / Delegation Memory

B_AWM->>B_AOE: 会话超时，停止旧工作流
B_AWM->>B_AWM: 工作流引用次数自减
B_AWM->>B_AOE: 确认停止
deactivate B_AWM

A_AOE->>A_AWM: 生成并启动新工作流<br>(含新远端接口)
activate A_AWM

A_AWM->>A_AOE: 新工作流启动成功
A_AWM->>A_MC: 更新 workflow_summary<br>记录 failover 结果
A_MC->>A_AWM: 更新成功

deactivate A_AWM
```

说明：

* A 侧记录“调用 B 失败，并切换到 C”。
* B 侧也记录“我的 Agent 被 A 调用时失败，失败原因是什么”。
* C 侧如果执行成功，也会保存本地 Agent 远端调用经验。
* 重编排时 MC 的历史失败经验可以影响新的 Agent 选择。

---

## 5. 单主体工作流停止编排

```mermaid
sequenceDiagram
autonumber

participant APP as 1.应用管理层<br>(APP)

box 智能体编排层
    participant AOE as 编排引擎<br>(AOE)
    participant AWM as 工作流管理<br>(AWM)
    participant ASD as 调度部署<br>(ASD)
    participant ARDC as 注册发现<br>(ARDC)
    participant MC as 记忆中心<br>(MC)
end

box 运行层
    participant ALCM as 生命周期管理<br>(ALCM)
    participant POD as Agent 沙箱<br>(POD)
end

participant STORE as 本体记忆库
participant RES as 资源抽象层<br>(Resource)

APP->>AOE: 停止编排
AOE->>AOE: 检出任务图
AOE->>AWM: 停止工作流

AWM->>MC: collect_all_outboxes(workflow_id)
MC->>POD: 回收 Agent 输出目录
POD->>MC: 返回 output 记忆候选

MC->>MC: 写入候选记忆<br>(task_result, agent_experience)
MC->>AWM: 写入成功

AWM->>MC: 提交停止前工作流摘要<br>(workflow_summary / interrupted_summary)
MC->>STORE: 写入本体长期记忆
STORE->>MC: 写入成功

AWM->>MC: 关闭 workflow memory session
MC->>AWM: 关闭成功

AWM->>AWM: 工作流引用次数自减
AWM->>ALCM: 退订智能体
ALCM->>ALCM: 智能体引用次数自减
ALCM->>AWM: 确认退订

AWM->>AOE: 确认停止

AOE->>ASD: 关闭智能体
ASD->>ALCM: 关闭智能体
ALCM->>POD: 销毁 Agent 沙箱
POD->>ALCM: 销毁成功

ALCM->>RES: 释放资源
RES->>ALCM: 确认释放

ALCM->>ASD: 确认关闭
ASD->>ARDC: 注销智能体实例
ARDC->>ASD: 确认注销

ASD->>AOE: 确认关闭
AOE->>APP: 确认停止
```

说明：

* Pod 销毁前必须先由 MC 回收 output。
* MC 关闭 workflow memory session 后，沙箱专属记忆目录可以归档或删除。
* 归档策略由 `memory_policy` 决定。

---

## 6. 跨主体工作流停止编排

```mermaid
sequenceDiagram
autonumber

box 设备 A / 调用方
    participant A_APP as 应用管理层<br>(APP_A)
    participant A_AOE as 编排引擎<br>(AOE_A)
    participant A_AWM as 工作流管理<br>(AWM_A)
    participant A_ASD as 调度部署<br>(ASD_A)
    participant A_ARDC as 注册发现<br>(ARDC_A)
    participant A_MC as 记忆中心<br>(MC_A)
end

box 设备 B / 被调用方
    participant B_AOE as 编排引擎<br>(AOE_B)
    participant B_AWM as 工作流管理<br>(AWM_B)
    participant B_ASD as 调度部署<br>(ASD_B)
    participant B_ARDC as 注册发现<br>(ARDC_B)
    participant B_MC as 记忆中心<br>(MC_B)
    participant B_POD as Agent 沙箱<br>(POD_B)
end

A_APP->>A_AOE: 停止编排
A_AOE->>A_AOE: 检出任务图

rect rgb(200,250,220)
    A_AOE->>A_AWM: 停止本地工作流

    A_AWM->>A_MC: 回收本地 Agent 输出
    A_MC->>A_AWM: 回收成功

    A_AWM->>A_MC: 提交本地 workflow_summary
    A_MC->>A_AWM: 提交成功

    A_AWM->>A_MC: 关闭本地 workflow memory session
    A_MC->>A_AWM: 关闭成功

    A_AWM->>A_AWM: 工作流引用次数自减
    A_AWM->>A_ASD: 请求关闭本地智能体
    A_ASD->>A_ARDC: 注销本地智能体实例
    A_ARDC->>A_ASD: 确认注销
    A_ASD->>A_AWM: 本地关闭成功
end

rect rgb(200,250,120)
    A_AOE->>B_AOE: 停止远端工作流<br>(session_id, caller_workflow_id)

    B_AOE->>B_AWM: 停止远端工作流
    B_AWM->>B_MC: 回收远端 Agent 输出<br>(bundle_id)
    B_MC->>B_POD: 收集 /sandbox/memory/output
    B_POD->>B_MC: 返回 result + writeback_to_local + writeback_to_caller

    B_MC->>B_MC: 保存 writeback_to_local<br>本地 Agent 远端调用经验
    B_MC->>B_AWM: 本地保存成功

    B_AWM->>B_AOE: 返回 writeback_to_caller
    B_AOE->>A_AOE: 返回远端停止确认 + writeback_to_caller

    B_AWM->>B_AWM: 工作流引用次数自减
    B_AWM->>B_ASD: 请求关闭远端智能体
    B_ASD->>B_ARDC: 注销远端智能体实例
    B_ARDC->>B_ASD: 确认注销
    B_ASD->>B_AWM: 远端关闭成功
end

A_AOE->>A_MC: 保存远端 writeback_to_caller
A_MC->>A_MC: 写入 remote_call_result / workflow_summary

A_AOE->>A_MC: 合并本地 + 远端停止摘要
A_MC->>A_AOE: 合并成功

A_AOE->>A_APP: 确认停止
```

说明：

* 远端停止时，B_MC 必须先保存本地 Agent 远端调用经验。
* B_MC 再将调用方可见的 writeback 返回 A_MC。
* A_MC 保存“我调用远端能力的结果”。
* B_MC 保存“我的 Agent 被远端调用的经验”。

---

## 7. MC 记忆包管理流程

```mermaid
sequenceDiagram
autonumber

participant AOE as 编排引擎<br>(AOE)
participant AWM as 工作流管理<br>(AWM)
participant MC as 记忆中心<br>(MC)
participant STORE as 记忆库<br>(Memory Store)
participant ASD as 调度部署<br>(ASD)
participant POD as Agent 沙箱<br>(POD)

AOE->>MC: create_memory_bundle<br>(task_id, agent_id, scope, policy)
MC->>STORE: 检索相关 User Memory
STORE->>MC: 返回用户记忆候选

MC->>STORE: 检索相关 Agent Memory
STORE->>MC: 返回 Agent 经验候选

MC->>MC: 过滤、摘要、脱敏、排序
MC->>MC: 生成 bundle manifest
MC->>MC: 生成专属目录<br>/memory-bundles/{workflow_id}/{agent_instance_id}

MC->>AOE: 返回 memory_mount_spec

AOE->>AWM: 下发 memory_bundle_refs
AWM->>ASD: 部署 Agent<br>(memory_mount_spec)

ASD->>POD: 挂载 /sandbox/memory

POD->>POD: 读取 input/
POD->>POD: 读取 local/
POD->>POD: 写入 output/

AWM->>MC: collect_outbox(bundle_id)
MC->>POD: 拉取 output/
POD->>MC: 返回 output 文件

MC->>MC: 按 target_owner 分流
MC->>STORE: 写入本地长期记忆
MC->>AWM: 返回 writeback_to_caller
```

---

## 8. Agent 沙箱记忆目录规范

每个 Agent 实例只允许访问自己的 `/sandbox/memory`。

```text
/sandbox/memory/
├── manifest.json
├── input/
│   ├── task.md
│   ├── delegated_context.json
│   ├── constraints.json
│   ├── policy.json
│   └── caller_info.json
│
├── local/
│   ├── agent_profile.json
│   ├── agent_memory_excerpt.json
│   └── device_public_context.json
│
└── output/
    ├── result.json
    ├── execution_notes.md
    ├── writeback_to_caller.jsonl
    ├── writeback_to_local.jsonl
    └── artifacts/
```

### 8.1 input 目录

由 MC 生成，Agent 只读。

```text
input/task.md
```

当前任务描述。

```text
input/delegated_context.json
```

调用方允许暴露的上下文。单主体任务中可为空。

```text
input/policy.json
```

当前任务的记忆读写策略。

```text
input/caller_info.json
```

调用方设备、Workflow、Session 信息。单主体任务中 caller 与本设备相同。

---

### 8.2 local 目录

由被调用方本地 MC 生成，Agent 只读。

```text
local/agent_profile.json
```

当前 Agent 的身份、能力、版本、约束。

```text
local/agent_memory_excerpt.json
```

MC 为当前任务抽取的 Agent 本地经验。

```text
local/device_public_context.json
```

本设备允许公开给本地 Agent 的上下文。

---

### 8.3 output 目录

由 Agent 写入，MC 回收。

```text
output/result.json
```

任务执行结果。

```text
output/writeback_to_caller.jsonl
```

返回给调用方 MC 的候选记忆。

```text
output/writeback_to_local.jsonl
```

写回本地 Agent Memory 的候选记忆。

---

## 9. MemoryBundle 数据结构

```json
{
  "bundle_id": "mb_xxx",
  "owner_device_id": "device_A",
  "owner_user_id": "user_A",
  "workflow_id": "wf_xxx",
  "task_id": "task_001",
  "agent_id": "vision_agent_001",
  "agent_instance_id": "inst_xxx",
  "mount_path": "/sandbox/memory",
  "visibility": "local",
  "created_at": "2026-06-10T10:00:00Z",
  "expires_at": "2026-06-10T12:00:00Z",
  "policy": {
    "input_readonly": true,
    "output_collect": true,
    "allow_agent_read_local_memory": true,
    "allow_agent_write_local_memory": true,
    "allow_writeback_to_caller": false,
    "allow_save_raw_input": false
  }
}
```

---

## 10. DelegatedMemoryBundle 数据结构

跨设备调用时使用。

```json
{
  "delegation_id": "dmb_xxx",
  "caller_device_id": "device_A",
  "caller_user_id": "user_A",
  "caller_workflow_id": "wf_A_001",
  "caller_task_id": "task_A_003",
  "callee_device_id": "device_B",
  "target_agent_id": "vision_agent_B",
  "remote_session_id": "sess_B_001",
  "delegated_context": {
    "task_summary": "请对脱敏后的目标区域进行视觉识别",
    "allowed_memories": [],
    "constraints": {}
  },
  "policy": {
    "allow_callee_local_memory_read": true,
    "allow_callee_save_execution_experience": true,
    "allow_callee_save_caller_identity": true,
    "allow_callee_save_delegated_context": false,
    "allow_callee_save_raw_input": false,
    "caller_writeback_allowed": true,
    "local_writeback_allowed": true
  },
  "expires_at": "2026-06-10T12:00:00Z"
}
```

---

## 11. DistributedState v2 新增字段

```python
class DistributedState(TypedDict):
    # 原有字段略

    # 记忆开关
    memory_enabled: bool

    # 当前本体
    device_id: str
    user_id: str

    # 当前 Workflow 记忆域
    memory_scope: dict

    # Planner 可见的摘要级记忆
    planner_memory_context: list[dict]

    # 每个 task / agent 的记忆包引用
    memory_bundles: dict[str, dict]

    # 跨设备委派记忆包
    delegated_memory_bundles: dict[str, dict]

    # Agent 沙箱执行后的写回
    memory_writebacks: list[dict]

    # 记忆策略
    memory_policy: dict
```

示例：

```json
{
  "memory_enabled": true,
  "device_id": "device_A",
  "user_id": "user_A",
  "memory_scope": {
    "owner_device_id": "device_A",
    "owner_user_id": "user_A",
    "app_id": "app_vehicle_decision",
    "workflow_id": "wf_001"
  },
  "memory_bundles": {
    "task_001": {
      "bundle_id": "mb_001",
      "agent_id": "vision_agent_001",
      "agent_instance_id": "inst_abc",
      "mount_path": "/sandbox/memory",
      "visibility": "local",
      "input_readonly": true,
      "output_collect": true
    }
  },
  "delegated_memory_bundles": {
    "remote_task_002": {
      "delegation_id": "dmb_001",
      "target_device_id": "device_B",
      "target_agent_id": "vision_agent_B",
      "visibility": "delegated",
      "expires_at": "2026-06-10T12:00:00Z"
    }
  }
}
```

---

## 12. MC 接口定义

### 12.1 本地记忆包接口

| 方法     | 路径                                          | 说明                  |
| ------ | ------------------------------------------- | ------------------- |
| POST   | `/memory/bundles/create`                    | 创建 MemoryBundle     |
| POST   | `/memory/bundles/{bundle_id}/materialize`   | 将记忆包物化为沙箱目录         |
| GET    | `/memory/bundles/{bundle_id}/download`      | initContainer 下载记忆包 |
| POST   | `/memory/bundles/{bundle_id}/upload_outbox` | 上传 Agent output     |
| POST   | `/memory/bundles/{bundle_id}/commit`        | 提交记忆包输出             |
| DELETE | `/memory/bundles/{bundle_id}`               | 删除或归档记忆包            |

---

### 12.2 跨设备委派接口

| 方法     | 路径                                              | 说明             |
| ------ | ----------------------------------------------- | -------------- |
| POST   | `/memory/delegations/create`                    | 调用方创建委派记忆包     |
| POST   | `/memory/delegations/accept`                    | 被调用方接收委派记忆包    |
| POST   | `/memory/delegations/{delegation_id}/writeback` | 被调用方向调用方写回候选记忆 |
| DELETE | `/memory/delegations/{delegation_id}`           | 删除或关闭委派会话      |

---

### 12.3 记忆查询接口

| 方法     | 路径                    | 说明                |
| ------ | --------------------- | ----------------- |
| POST   | `/memory/search`      | 查询本体记忆 / Agent 记忆 |
| POST   | `/memory/write`       | 写入本地记忆            |
| POST   | `/memory/batch_write` | 批量写入本地记忆          |
| GET    | `/memory/{memory_id}` | 查询单条记忆            |
| DELETE | `/memory/{memory_id}` | 删除记忆              |

---

## 13. ASD / ALCM 部署接口新增参数

原部署接口：

```text
部署智能体(镜像ID, 资源配置)
```

v2 增强为：

```text
部署智能体(镜像ID, 资源配置, memory_mount_spec)
```

示例：

```json
{
  "agent_id": "vision_agent_001",
  "image_id": "image_vision",
  "resource_config": {
    "cpu_cores": 2,
    "memory_mb": 4096,
    "gpu_count": 1
  },
  "memory_mount": {
    "bundle_id": "mb_001",
    "mount_path": "/sandbox/memory",
    "input_mode": "readonly",
    "output_mode": "collect",
    "collector": "sidecar"
  }
}
```

---

## 14. K8s 实现方式

```mermaid
sequenceDiagram
autonumber

participant ASD as ASD
participant MC as MC
participant K8S as K8s API
participant INIT as initContainer
participant AGENT as Agent 容器
participant SIDE as sidecar collector

ASD->>MC: create_bundle(agent_instance_id, task_id)
MC->>ASD: 返回 bundle_id

ASD->>K8S: 创建 Pod<br>(emptyDir + initContainer + sidecar)

K8S->>INIT: 启动 initContainer
INIT->>MC: pull_bundle(bundle_id)
MC->>INIT: 返回 bundle 文件
INIT->>INIT: 写入 /sandbox/memory/input

K8S->>AGENT: 启动 Agent 容器
AGENT->>AGENT: 读取 /sandbox/memory/input
AGENT->>AGENT: 写入 /sandbox/memory/output

K8S->>SIDE: sidecar 监听 output
SIDE->>MC: upload_outbox(bundle_id)
MC->>MC: 暂存、审核、提交候选记忆
```

优点：

```text
1. Agent 不需要直接访问 MC 全量存储
2. 每个 Pod 的 memory 目录天然隔离
3. Pod 销毁后 output 已由 sidecar 回收
4. 不依赖跨 Pod 共享文件系统
```

## 15. 记忆写回策略

### 15.1 写回调用方

文件：

```text
/sandbox/memory/output/writeback_to_caller.jsonl
```

用途：

```text
返回调用方 MC，用于保存远端调用结果、远端执行经验、工作流摘要。
```

示例：

```json
{
  "memory_type": "remote_call_result",
  "target_owner": "caller",
  "content": "device_B 的 vision_agent_B 完成目标检测任务，结果可靠，耗时 2.3s。",
  "confidence": 0.91
}
```

---

### 15.2 写回本地 Agent

文件：

```text
/sandbox/memory/output/writeback_to_local.jsonl
```

用途：

```text
写回被调用设备本地 MC，保存本地 Agent 的远端调用经验。
```

示例：

```json
{
  "memory_type": "agent_remote_experience",
  "target_owner": "local_agent",
  "agent_id": "vision_agent_B",
  "caller_device_id": "device_A",
  "caller_workflow_id": "workflow_A_001",
  "content": "vision_agent_B 被 device_A 的 workflow_A_001 调用执行目标检测任务，输入为脱敏上下文，执行成功，耗时 2.3s。"
}
```

---

## 16. 记忆权限策略

远端调用的默认策略：

```json
{
  "allow_callee_local_memory_read": true,
  "allow_callee_save_execution_experience": true,
  "allow_callee_save_caller_identity": true,
  "allow_callee_save_delegated_context": false,
  "allow_callee_save_raw_input": false,
  "caller_writeback_allowed": true,
  "local_writeback_allowed": true
}
```

含义：

```text
1. 允许被调用 Agent 读取自己的本地经验。
2. 允许被调用设备保存“被谁调用、执行了什么、结果如何”。
3. 不允许被调用设备保存调用方未授权的私有原始上下文。
4. 调用方可接收远端执行结果和候选记忆。
5. 被调用方可保存本地 Agent 的执行经验。
```