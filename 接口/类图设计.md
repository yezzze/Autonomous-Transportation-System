...
classDiagram
    ...
    class AgentHNSWIndex {
        ...
        +Dict agent_metadata
        +Dict id_to_idx
        +add_agent(agent) bool
        +add_agents(agents, rebuild) int
        +search(query, top_k) List~Tuple~
        +remove_agent(agent_id) bool
        +update_agent(agent) bool
        +save_index() None
        +_get_agent_text(agent) str
    }
    ...
    AgentRegistryClient --> AgentHNSWIndex : 大规模场景使用
...
- `AgentHNSWIndex`: 基于 HNSW 算法的向量索引
  - 使用 `sentence-transformers` 编码能力描述
  - 支持 10 万级智能体的快速检索
...
| `src/service/agent_hnsw_index.py` | `AgentHNSWIndex` | HNSW向量检索 |
