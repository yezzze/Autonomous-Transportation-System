...
环境变量:
  ...
  GROUND_TRUTH              : 可选，包含 id/truth 列的分类真值 CSV 路径
...
# 业务运行时：(config, cloud_runtime)，由 lifespan 初始化。
_runtime = None

COARSE_EVENT_TYPE_MAP = {
    "C01": "C1",
    "C02": "C2",
    "C03": "C2",
    "C04": "C2",
...
def _coarse_event_type(raw: str) -> str:
    """与 evaluate_targeted_90.py 一致地转换粗粒度事件类别。"""
    return COARSE_EVENT_TYPE_MAP.get(str(raw).strip().upper(), "OTHER")


def _event_type_from_description(description: object) -> str:
    """从模型的结构化 description 中提取 event_type，失败时返回 NONE。"""
    candidate = str(description or "").strip().lstrip("\ufeff")
    ...
    if not isinstance(parsed, dict):
        return "NONE"
    return str(parsed.get("event_type") or "NONE").strip().upper()
...
def _evaluate_ground_truth(
    ...
    """读取真值 CSV，并按 targeted-90 的粗分类规则评价本次输出。"""
    try:
        with ground_truth_path.open("r", encoding="utf-8-sig", newline="") as handle:
            truth_index = {
                str(row.get("id") or "").strip(): str(row.get("truth") or "")
                .strip()
                .upper()
        ...
        raise ValueError(f"无法读取 GROUND_TRUTH={ground_truth_path}: {exc}") from exc

    raw_event_type = _event_type_from_description(description)
    coarse_event_type = _coarse_event_type(raw_event_type)
    truth = truth_index.get(sample_id)
    return {
        "sample_id": sample_id,
        ...
        "correct": coarse_event_type == truth if truth is not None else False,
    }
...
async def lifespan(_: FastAPI):
    ...
    try:
        ...
        # 模型加载：云端 Qwen2.5-VL 理解运行时。
        config = load_config(RUNTIME_CONFIG_PATH, role="cloud")
        cloud_runtime = build_cloud_runtime(config)
        _runtime = (config, cloud_runtime)
        logger.info(
            "Cloud Qwen runtime loaded (mock=%s)",
            config["runtime"]["mock"],
        )
    except Exception as exc:
...
async def agent_function(
    ...
    # TODO: 在此处编写你的业务逻辑
    # ─── 开发者替换区: 在此处添加你的业务逻辑 ───
    # 云端：从 NATS 获取路端 Token 载荷，零填充恢复并生成结构化事故描述。
    stage_started = time.monotonic()
    try:
        config, cloud_runtime = _runtime
        payload_in = decode_data.get("processed_data", {})
        if not isinstance(payload_in, dict):
            payload_in = decode_data
        sample_id = str(
            payload_in.get("sample_id") or decode_data.get("sample_id") or "a2a_task"
        )
        task_type = str(
        ...
        if task_type == "baseline":
            # 传统链路：云端对完整视频全量分析
            video_base64 = payload_in.get("video_base64") or parameters.get(
                "video_base64"
            )
            video_path = parameters.get("video_path") or payload_in.get("video_path")
            temp_video = None
            if video_base64:
                work_dir = Path(config["paths"]["work_dir"])
                work_dir.mkdir(parents=True, exist_ok=True)
                temp_video = work_dir / "a2a_full_video.mp4"
                temp_video.write_bytes(base64.b64decode(video_base64))
                video_path = temp_video
            if not video_path:
                raise ValueError("cloud_video 需要 video_path 或 video_base64")
            input_video = Path(str(video_path))
            if not input_video.exists():
                raise FileNotFoundError(f"video_path 不存在：{input_video}")
            try:
                output, timings = cloud_runtime.process_video(input_video, sample_id)
            finally:
                if temp_video is not None:
                    temp_video.unlink(missing_ok=True)
        else:
            # 被测链路：恢复 Token 并生成结构化描述
            token_text = payload_in.get("token_payload") or decode_data.get("token_payload")
            if not isinstance(token_text, str) or not token_text:
                raise ValueError("上游数据缺少 token_payload 字符串")
            token_payload = json.loads(token_text)
            output, timings = cloud_runtime.process_tokens(token_payload, sample_id)

        ground_truth_env = os.getenv("GROUND_TRUTH", "").strip()
        if ground_truth_env:
            ground_truth_evaluation = _evaluate_ground_truth(
                Path(ground_truth_env),
                sample_id,
                output.get("description"),
            )
            output["ground_truth_evaluation"] = ground_truth_evaluation
            observe_performance_metrics(
                {"correct": float(bool(ground_truth_evaluation["correct"]))}
            )
        processed = {"output": output, "timings_ms": timings}
        result = {
            "status": "success",
        ...
        if task_type == "baseline":
            observe_performance_metrics(
                {
                    "cloud_video_generation_ms": float(
                        timings.get("cloud_full_model_generation", 0.0)
                    ),
                    "event_detected": float(bool(output.get("event_detected"))),
                }
            )
        else:
            observe_performance_metrics(
                {
                    "cloud_generation_ms": float(
                        timings.get("cloud_token_model_generation", 0.0)
                    ),
                    "event_detected": float(bool(output.get("event_detected"))),
                }
            )
    ...
    # 云端是链路终点：不再有下游 NATS 输出，必须把业务结果返回给调用方。
    return result
...
def _build_agent_card() -> AgentCard:
    ...
    skill = AgentSkill(
        id="cloud_vlm_accident_description",
        name="Cloud VLM Accident Description Agent",
        description=(
            "Restores compressed visual tokens from the road agent with "
            "zero-fill and generates structured accident descriptions with "
            "Qwen2.5-VL."
        ),
        input_modes=["application/json", "text/plain"],
        output_modes=["text/plain"],
        tags=["cloud-agent", "accident-understanding"],
        examples=[
            "Understand the accident from upstream tokens",
            (
                '{"task_description": "Understand the accident", '
                '"parameters": {"source_cluster": "vrc"}, '
                '"metadata": {}}'
            ),
        ],
    )
    ...
    return AgentCard(
        name="Cloud VLM Accident Description Agent",
        description=(
            "云端 Agent：零填充恢复路端 Token，Qwen2.5-VL 生成结构化事故描述。"
        ),
        version="1.1.0",
...
app = FastAPI(title="Cloud VLM Accident Description Agent", lifespan=lifespan)
