...
def img_geom_transform(imgs, post_rot, post_tran,
                       resize, resize_dims, crop,
                       flip, rotate):
    """
    仅进行几何变换 (resize, crop, flip, rotate)
    不改变颜色、不归一化，用于生成不过曝的 origin_img。
    """
    imgs_output = []
    for img in imgs:
        # --- 几何增强 ---
        img = img.resize(resize_dims, Image.BILINEAR)  # 尺寸缩放
        img = img.crop(crop)  # 裁剪
        if flip:
            img = img.transpose(method=Image.FLIP_LEFT_RIGHT)  # 左右翻转
        img = img.rotate(rotate, resample=Image.BILINEAR)  # 旋转
        imgs_output.append(img)
    # --- 同步几何变换矩阵 ---
...
def coord_3d_to_2d(gt_box3d, int_matrix, ext_matrix, image_H=600, image_W=800, image=None, idx=None):
    ...
    if DEBUG:
        ...
        plt.clf()
        plt.imshow(fg_mask)
        plt.savefig(f"/GPFS/rhome/yifanlu/OpenCOOD/vis_result/dairv2x_lss_vehonly/image_gt_box2d_{idx}_mask.png",
                    dpi=300)
        plt.clf()
...
def draw_coord_3d_to_2d(gt_box3d, int_matrix, ext_matrix, image_H=600, image_W=800, image=None, idx=None, post_rot=None,
                        post_tran=None):
    """
    Projects XYZ points onto the canvas and returns the projected canvas
    coordinates.
    Args:
        ...
        ext_matrix : np.ndarray
            shape (4, 4), T_wc, transform point in camera coord to world coord.
        post_rot : np.ndarray, optional
            shape (3, 3), post augmentation rotation matrix
        post_tran : np.ndarray, optional
            shape (3,), post augmentation translation vector
    Returns:
        gt_box2d : np.ndarray
            shape (N, 8, 2). pixel coord (u, v) in the image. You may want to flip them for image data indexing.
        gt_box2d_mask : np.ndarray (bool)
    ...
    xyz_hom = np.concatenate(
        [xyz, np.ones((xyz.shape[0], 1), dtype=np.float32)], axis=1)
    # Ensure int_matrix is 3x3
    if int_matrix.shape == (4, 4):
        int_matrix = int_matrix[:3, :3]

    # Transform from LiDAR to camera coordinates
    ext_matrix_inv = np.linalg.inv(ext_matrix)[:3, :4]

    # Project to camera coordinates: [x_cam, y_cam, z_cam]
    cam_pts = (ext_matrix_inv @ xyz_hom.T).T  # [N*8, 3]

    # Project to image plane: K @ [x_cam, y_cam, z_cam]
    img_pts = (int_matrix @ cam_pts.T).T  # [N*8, 3]

    depth = img_pts[:, 2]
    uv = img_pts[:, :2] / (depth[:, None] + 1e-6)  # [N*8, 2]

    # Apply post-augmentation transformation if provided
    if post_rot is not None and post_tran is not None:
        # Convert to homogeneous coordinates
        uv_hom = np.concatenate([uv, np.ones((uv.shape[0], 1), dtype=np.float32)], axis=1)  # [N*8, 3]
        # Apply post transformation: uv_aug = post_rot @ uv + post_tran
        uv = (post_rot[:2, :2] @ uv.T).T + post_tran[:2]  # [N*8, 2]
