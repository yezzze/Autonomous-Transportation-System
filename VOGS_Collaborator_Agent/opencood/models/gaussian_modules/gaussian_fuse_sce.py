...
from opencood.models.sce_models.zbuffer_blind import generate_ego_depth_map, zbuffer_culling
...
def transform_neighbor_gaussians(
    ...
    ego_GsSCE = GsSCE[0] if GsSCE is not None else None

    # Pre-generate Ego Depth Map for Z-Buffer Culling if enabled
    use_zbuffer_blind = fusion_args is not None and fusion_args.get("use_zbuffer_blind", False)
    ego_depth_map = None
    zbuffer_epsilon = 0.5
    projection_mat = None
    image_wh = None
    
    if use_zbuffer_blind and metas is not None and "projection_mat" in metas and "image_wh" in metas:
        projection_mat = metas["projection_mat"][b]  # [N_cams, 4, 4]
        image_wh = metas["image_wh"][b]              # [N_cams, 2]
        zbuffer_epsilon = fusion_args.get("zbuffer_epsilon", 0.5)
        
        # Only use valid ego Gaussians for generating the depth map
        x_min, y_min, z_min, x_max, y_max, z_max = roi_bounds
        ego_inside_roi = (
            (ego_mean[:, 0] > x_min) & (ego_mean[:, 0] < x_max - 1e-4) &
            (ego_mean[:, 1] > y_min) & (ego_mean[:, 1] < y_max - 1e-4) &
            (ego_mean[:, 2] > z_min) & (ego_mean[:, 2] < z_max - 1e-4)
        )
        ego_valid_mask = ego_inside_roi & (ego_opacity[:, 0] >= opacity_thresh)
        ego_valid_xyz = ego_mean[ego_valid_mask]
        
        ego_depth_map = generate_ego_depth_map(ego_valid_xyz, projection_mat, image_wh, kernel_size=fusion_args.get("zbuffer_kernel_size", 30))
    ...
    for j in range(1, n_cav):
        ...
        gs_raw = GaussianPrediction(
            means=mean.unsqueeze(0),
            scales=scale.unsqueeze(0),
            rotations=rot.unsqueeze(0),
            opacities=opacity.unsqueeze(0),
            semantics=semantic.unsqueeze(0),
        )
        gsSCE_raw = GsSCE[j].unsqueeze(0) if GsSCE is not None else None
        ...
        gs_roi = GaussianPrediction(
            means=mean_ego[valid_mask].unsqueeze(0),
            scales=scale[valid_mask].unsqueeze(0),
            rotations=rot_ego[valid_mask].unsqueeze(0),
            opacities=opacity[valid_mask].unsqueeze(0),
            semantics=semantic[valid_mask].unsqueeze(0),
        )
        gsSCE_roi = GsSCE[j][valid_mask].unsqueeze(0) if GsSCE is not None else None

        baseline_num = valid_mask.sum().item()
        comm_stats["baseline_num"] += baseline_num
        
        if ego_depth_map is not None:
            valid_idx = torch.where(valid_mask)[0]
            if len(valid_idx) > 0:
                sender_valid_xyz = mean_ego[valid_idx]
                blind_zone_mask = zbuffer_culling(
                    sender_xyz=sender_valid_xyz,
                    ego_depth_map=ego_depth_map,
                    projection_mat=projection_mat,
                    image_wh=image_wh,
                    epsilon=zbuffer_epsilon
                )
                valid_mask[valid_idx] = valid_mask[valid_idx] & blind_zone_mask
        
        gs_roiZbuffer = GaussianPrediction(
            means=mean_ego[valid_mask].unsqueeze(0),
            scales=scale[valid_mask].unsqueeze(0),
            rotations=rot_ego[valid_mask].unsqueeze(0),
            opacities=opacity[valid_mask].unsqueeze(0),
        ...
        gsSCE_roiZbuffer = GsSCE[j][valid_mask].unsqueeze(0) if GsSCE is not None else None
        
        if fusion_args is not None and fusion_args.get("enable_bandwidth_aware", False) and GsSCE is not None:
            agent_sce = GsSCE[j].squeeze(-1) # [num_anchor]
            target_ratio = fusion_args.get("target_bandwidth_ratio", 1.0)
            
            valid_mask = gumbel_topk_bandwidth_filter(agent_sce, valid_mask, target_ratio)

        gs_fullFilter = GaussianPrediction(
            means=mean_ego[valid_mask].unsqueeze(0),
            scales=scale[valid_mask].unsqueeze(0),
            rotations=rot_ego[valid_mask].unsqueeze(0),
            opacities=opacity[valid_mask].unsqueeze(0),
        ...
        gsSCE_fullFilter = GsSCE[j][valid_mask].unsqueeze(0) if GsSCE is not None else None

        filtered_mask = original_valid_mask & (~valid_mask)
        gs_filtered = GaussianPrediction(
            means=mean_ego[filtered_mask].unsqueeze(0),
            scales=scale[filtered_mask].unsqueeze(0),
            rotations=rot_ego[filtered_mask].unsqueeze(0),
            opacities=opacity[filtered_mask].unsqueeze(0),
            semantics=semantic[filtered_mask].unsqueeze(0),
        )
        gsSCE_filtered = GsSCE[j][filtered_mask].unsqueeze(0) if GsSCE is not None else None
        ...
        collab_dict[j-1] = {
            ...
            "gs_roi": gs_roi,
            "gsSCE_roi": gsSCE_roi,
            "gs_roiZbuffer": gs_roiZbuffer,
            "gsSCE_roiZbuffer": gsSCE_roiZbuffer,
            "gs_fullFilter": gs_fullFilter,
            "gsSCE_fullFilter": gsSCE_fullFilter,
            "gs_filtered": gs_filtered,
            "gsSCE_filtered": gsSCE_filtered,
        }

        if valid_mask.sum() == 0:
            continue  # no Gaussians in ROI for this agent, skip
            
        actual_num = valid_mask.sum().item()
        comm_stats["actual_num"] += actual_num
        ...
        if GsSCE is not None:
            shared_GsSCE_list.append(GsSCE[j][valid_mask])
    ...
    if len(shared_means_list) == 0:
        ...
        ), [], comm_stats, ego_GsSCE.unsqueeze(0) if ego_GsSCE is not None else None, collab_dict
    ...
    if GsSCE is not None:
        shared_GsSCE = torch.cat(shared_GsSCE_list, dim=0)
    ...
    ), num_of_gaussian_list, comm_stats, torch.cat([ego_GsSCE, shared_GsSCE], dim=0).unsqueeze(0) if GsSCE is not None else None, collab_dict
