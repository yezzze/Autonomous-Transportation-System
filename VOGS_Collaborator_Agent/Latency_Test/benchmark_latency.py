...
def test_parser():
    ...
    parser.add_argument("--model_dir", type=str, default="Latency_Test/ours/single_preSCE_noFrozen", help="Continued training path")
    ...
    parser.add_argument("--num_frames", type=int, default=1, help="Number of frames to run. 0=all frames")
    parser.add_argument("--warmup_frames", type=int, default=0, help="Number of warmup frames (excluded from timing)")
...
def main():
    ...
    print("Dataset Building")
    full_dataset = build_dataset(hypes, visualize=True, train=False)

    total_dataset_len = len(full_dataset)
    num_frames = opt.num_frames if opt.num_frames > 0 else total_dataset_len
    num_frames = min(num_frames, total_dataset_len)
    print(f"Total dataset: {total_dataset_len}, frames to run: {num_frames}, warmup: {opt.warmup_frames}")

    collate_fn = full_dataset.collate_batch_test
    modality_list = full_dataset.modality_name_list

    if num_frames < total_dataset_len:
        subset_dataset = Subset(full_dataset, list(range(num_frames)))
    else:
    ...
    # --- Priming: spawn workers + CUDA init before warmup ---
    print("Priming DataLoader and CUDA (1 batch)...")
    _prime_iter = iter(data_loader)
    _prime_batch = None
    try:
    ...
    if _prime_batch is not None:
        _prime_batch = train_utils.to_device(_prime_batch, device)
        _prime_batch["ego"]["benchmarking"] = True
        with torch.no_grad():
            if opt.fusion_method == "late":
                _ = inference_utils.inference_late_fusion(_prime_batch, model, full_dataset)
            elif opt.fusion_method == "early":
                _ = inference_utils.inference_early_fusion(_prime_batch, model, full_dataset)
            elif opt.fusion_method == "intermediate":
                _ = inference_utils.inference_intermediate_fusion(_prime_batch, model, full_dataset)
            elif opt.fusion_method in ("no", "no_w_uncertainty"):
                _ = inference_utils.inference_no_fusion(_prime_batch, model, full_dataset)
            elif opt.fusion_method == "single":
                _ = inference_utils.inference_no_fusion(_prime_batch, model, full_dataset, single_gt=True)
            elif opt.fusion_method == "late_heter":
                _ = inference_utils.inference_heter_late(_prime_batch, model, full_dataset)
            if torch.cuda.is_available():
                torch.cuda.synchronize()
        del _prime_batch
    del _prime_iter

    # Recreate data_loader so the iterator starts fresh for warmup
    data_loader = DataLoader(
        subset_dataset,
        batch_size=1,
        num_workers=24,
    ...
    for i, batch_data in pbar:
        ...
        with torch.no_grad():
            ...
            if opt.fusion_method == "late":
                infer_result = inference_utils.inference_late_fusion(batch_data, model, full_dataset)
            elif opt.fusion_method == "early":
                infer_result = inference_utils.inference_early_fusion(batch_data, model, full_dataset)
            elif opt.fusion_method == "intermediate":
                ...
                infer_result = inference_utils.inference_no_fusion(batch_data, model, full_dataset)
            elif opt.fusion_method == "no_w_uncertainty":
                infer_result = inference_utils.inference_no_fusion_w_uncertainty(batch_data, model, full_dataset)
            elif opt.fusion_method == "single":
                infer_result = inference_utils.inference_no_fusion(batch_data, model, full_dataset, single_gt=True)
            elif opt.fusion_method == "late_heter":
    ...
    if len(frame_times) > 0:
        ...
        print("\n" + "=" * 60)
        print(f"LATENCY BENCHMARK (warmup={opt.warmup_frames} excluded)")
        print("=" * 60)
        print(f"  Timed frames:    {len(frame_times)}")
        print(f"  Mean latency:    {frame_arr.mean()*1000:.2f} ms")
        print(f"  Std latency:     {frame_arr.std()*1000:.2f} ms")
        print(f"  Min latency:     {frame_arr.min()*1000:.2f} ms")
        print(f"  Max latency:     {frame_arr.max()*1000:.2f} ms")
        print(f"  Median latency:  {np.median(frame_arr)*1000:.2f} ms")
        print(f"  FPS (mean):      {1.0/frame_arr.mean():.2f}")
        print(f"  FPS (median):    {1.0/np.median(frame_arr):.2f}")
        print("=" * 60)
...
def eval_detection_result(
    opt, agent_modality_list, full_dataset, infer_result_single, result_stat, batch_data, idx, work_dir, hypes, i
):
    ...
    if getattr(full_dataset, "heterogeneous", False):
